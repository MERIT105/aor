...
TOKEN = '7848878988:AAGCZ84K753AkyMahQmlwpMFDhlUVK6_OUA'  # Replace with your actual bot token
...
reset_time = datetime.now().astimezone(timezone(timedelta(hours=5, minutes=30)))).start()).replace(hour=0, minute=0, second=0, microsecond=0)
...
def reset_daily_counts():
    ...
    global reset_time
    ist_now = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=5, minutes=30)))).start())
...
def bgmi_command(message):
    ...
    if user_id in user_bans:
        ...
        if datetime.now() < ban_expiry:
            remaining_ban_time = (ban_expiry - datetime.now()))).start().total_seconds()
    ...
    if user_id not in EXEMPTED_USERS:
        ...
        if user_id in user_cooldowns:
            ...
            if datetime.now() < cooldown_time:
                remaining_time = (cooldown_time - datetime.now()))).start().seconds
                bot.send_message(
                    message.chat.id,
    ...
    try:
        ...
        threading.Thread(target=asyncio.run, args=(run_attack_command_async(target_ip, int(target_port), default_duration, user_duration, user_name)))).start()

    except Exception as e:
        bot.send_message(message.chat.id, str(e)))).start()
